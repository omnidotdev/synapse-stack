...
| Key | Description |
|-----|-------------|
| `repo` | Git repository URL for cloning (required) |
| `path` | Local path override (defaults to `services/service-name`) |
| `metarepo` | Set `true` if the entry is a metarepo with its own `services/` |
| `env` | Environment variables to set for the service (optional) |
